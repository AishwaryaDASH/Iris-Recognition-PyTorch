...
class BaseModel(nn.Module):
	...
	def init_weights(self, ignore_backbone=False):
		print("[%s] Initialize weights..." % (self.__class__.__name__))
		for name, m in self.named_modules():
			if ignore_backbone and 'backbone' in name:
				continue
	...
	def load_pretrained_model(self, pretrained):
		if isinstance(pretrained, str):
			...
			print("[%s] Load pretrained model from state_dict" % (self.__class__.__name__))
