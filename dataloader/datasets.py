...
#	MMU2FolderDataset
#------------------------------------------------------------------------------
class MMU2FolderDataset(BaseDataset):

	_FORMAT = ('.jpg', '.png', '.bmp')

	def __init__(self, imgdir, num_classes, use_sigmoid=True, color_channel="RGB",
		...
		# Init BaseDataset
		super(MMU2FolderDataset, self).__init__(img_loader_mode=img_loader_mode, normalize_mode=normalize_mode)
	...
	def _get_image_files(self, imgdir):
		image_files = glob(os.path.join(imgdir, "*.*"), recursive=True)
	...
	def _get_labels(self):
		self.labels = [int(os.path.basename(file).split(".")[0][:-4])-1 for file in self.image_files]
...
#	MMU2TxtDataset
#------------------------------------------------------------------------------
class MMU2TxtDataset(BaseDataset):
	...
	def __init__(self, txtfile, num_classes, use_sigmoid=True, color_channel="RGB",
		...
		# Init BaseDataset
		super(MMU2TxtDataset, self).__init__(img_loader_mode=img_loader_mode, normalize_mode=normalize_mode)
